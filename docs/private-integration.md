...
Keep Blueprint and adapter patches in this private repository. Build patched applications in disposable, ignored checkouts under `.work/`. Keep public application branches and their release dependency graphs unchanged during this phase.
...
Blueprint continues to use `publish = false`. Patched application manifests must also disable publication. Nothing in this strategy publishes a crate, changes repository visibility, or uploads application binaries.

## Private checkout and local dependency

Clone this private repo using the owner's normal GitHub authentication (for example `gh repo clone piny4man/blueprint`). Credentials stay with Git/gh; never place tokens in Cargo.toml, a URL, or a patch.

Use the reviewed Blueprint commit for a reproducible build. A local path dependency then avoids registry publication and extra private Git fetching during Cargo resolution. Record the Blueprint commit, application base SHA, applied patch and application Cargo.lock together in each integration's validation notes.

There is no public registry fallback for `blueprint-theme`. A missing local checkout is an actionable setup error. An optional Cargo feature would not make an inaccessible private dependency an acceptable public manifest dependency: dependency resolution/packaging still needs consideration. We therefore keep the entire adapter change private for now.

## Disposable app layout

From the Blueprint root, check out an app at the full SHA recorded in its integration record. For example, after `integration/tablero/base-revision` and `adapter.patch` have been added by NUKE-72:
...
| App manifest | Blueprint dependency path |
|---|---|
...
```toml
blueprint = { package = "blueprint-theme", path = "../../../.." }
```
...
`integration/private-consumer` is an independent Cargo project with its own lockfile and `publish = false`. It imports this private library using the alias `blueprint`, loads the theme from disk, and checks shared appearance, explicit override precedence and standalone defaults.
...
Add `--offline` once dependencies are cached to demonstrate that Cargo does not need private Git or registry publication for Blueprint. A fresh machine still needs ordinary public dependencies from crates.io. The consumer is intentionally not a GUI app and does not prove font installation, rendering or laptop performance.
...
Before moving an adapter upstream, decide separately whether to publish an appropriately licensed Blueprint crate, expose a public interchange interface, or continue keeping integrations private. Verify the registry name and licensing only when publication is actually chosen. Existing GPL application licensing is preserved and requires consideration before distributing modified builds.
...
Validated 8 September 2026 on Linux with Rust 1.98.1 against Blueprint's merged
foundation commit `3166fb054a595fb9a86443d225b428a64f8eadb4` (library source unchanged).
...
- Cargo metadata reports a local path dependency with no registry/Git source for Blueprint and publishing disabled for the consumer.
